...
def square(square_in):
    ...
    if "error" in results:
        ...
        return render_template("results.html", determinant=results["determinant"], n=len(results["original matrix"]), m=len(results["inhomogeneous part"]),inhomogeneous_part=results["inhomogeneous part"], original_matrix=results["original matrix"])
